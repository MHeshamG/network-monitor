...
StompError StompFrame::ParseFrame(const std::string_view frame)
{
    ...
    auto command{ToCommand(plain.substr(commandStart, commandEnd))};
    ...
    Headers headers{};
    while (headerLineStart < plain.size() && plain.at(headerLineStart) != newLine)
    {
        ...
        auto header{ToHeader(plain.substr(headerStart, headerEnd - headerStart))};
