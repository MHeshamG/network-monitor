    ...
    */
    ...
    public:
            ...
            const NetworkMonitorConfig &config)
        {
            ...
            auto networkLayoutFile{config.networkLayoutFile.empty() ? std::filesystem::temp_directory_path() / "network-layout.json" : config.networkLayoutFile};
            if (config.networkLayoutFile.empty())
            {
                spdlog::info(
    ...
    private:
        ...
        void OnNetworkEventsConnect(StompClientError ec)
        {
