...
MockWebSocketClient::MockWebSocketClient(
    ...
    boost::asio::ssl::context &ctx) : context_{boost::asio::make_strand(ioc)}
{
    ...
    else
    {
        ...
        boost::asio::post(
            ...
            [this, onConnect]()
            {
                ...
                if (onConnect)
                {
                    std::cout<<".........."<<connectEc.message()<<".........."<<std::endl;
                    onConnect(connectEc);
                }
            });
        boost::asio::post(
...
MockWebSocketClientForStomp::MockWebSocketClientForStomp(
    ...
    const std::string &port,
    boost::asio::io_context &ioc,
    boost::asio::ssl::context &ctx) : MockWebSocketClient(url, endpoint, port, ioc, ctx)
{
