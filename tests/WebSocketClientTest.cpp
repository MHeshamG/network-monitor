...
BOOST_AUTO_TEST_CASE(fail_tls_handshake, *timeout{1})
{
    ...
    // Set the expected error codes.
    MockTlsStream::handshakeEc = boost::asio::ssl::error::stream_truncated;
    ...
    auto onConnect{[&calledOnConnect](auto ec)
                   {
                       calledOnConnect = true;
                       BOOST_CHECK_EQUAL(ec, boost::asio::ssl::error::stream_truncated);
                   }};
    client.Connect(onConnect);
    ioc.run();
...
BOOST_AUTO_TEST_CASE(fail_websocket_handshake, *timeout{1})
{
    ...
    // Set the expected error codes.
    MockTlsWebSocketStream::handshakeEc = boost::beast::websocket::error::upgrade_declined;
    ...
    auto onConnect{[&calledOnConnect](auto ec)
                   {
                       calledOnConnect = true;
                       BOOST_CHECK(ec = boost::beast::websocket::error::upgrade_declined);
                   }};
    client.Connect(onConnect);
    ioc.run();
...
BOOST_AUTO_TEST_CASE(two_message, *timeout{1})
{
    // We use the mock client so we don't really connect to the target.
    const std::string url{"some.echo-server.com"};
    const std::string endpoint{"/"};
    ...
    auto onMessage = [&messagesReceived, &client, &message](auto ec, auto msg)
    {
        ++messagesReceived;
    ...
    // When we get here, the io_context::run function has run out of work to do.
    BOOST_CHECK_EQUAL(messagesReceived, 2);
}
...
BOOST_FIXTURE_TEST_SUITE(nl1, WebSocketClientTestFixture);
...
BOOST_AUTO_TEST_CASE(close, *timeout {1})
{
    ...
    auto onClose{[&calledOnClose,&client](auto ec)
                   {
                       calledOnClose = true;
                       BOOST_CHECK(!ec);
                   }};
    client.Connect([&client, &onClose](auto ec) {
        BOOST_REQUIRE(!ec);
